...
}
{
    ...
    struct rmq {
        ...
        rmq(const vector<T>& a, const func& _F)
            : F(_F)
        {
            n = (int)a.size();
            int log_size = 0;
...
}
{
    ...
    void HLD(int u, int pa)
    {
        ...
        for (int v : adj[u]) {
            if (v == pa)
                continue;
            if (dinh == 0 || numNode[v] > numNode[dinh])
                dinh = v; ////lấy đỉnh v có số nút con là max
        }
        if (dinh > 0)
            HLD(dinh, u);
        for (int v : adj[u]) {
    ...
    { /// Tách đường đi từ u đến a: đỉnh u thuộc cây con gốc a
...
}
{
    ...
    template <typename T>
    vector<vector<T>>
    operator*(const vector<vector<T>>& a, const vector<vector<T>>& b) {
...
}
{
    ...
    Circle MEC_small(vector<Point> p)
    {
        ...
        if ((int)p.size() == 1)
            return Circle(p[0], 0);
        if ((int)p.size() == 2)
            return circle_two(p[0], p[1]);

        for (int i = 0; i < 3; i++) {
            for (int j = i + 1; j < 3; j++) {
                Circle C = circle_two(p[i], p[j]);
                if (check_valid_circle(C, p))
                    return C;
            }
        }
...
struct Ford_FulkersonFlow { /// Base 0-th, The vertices are numbered from 0 to n - 1
    ...
    { /// if flow > int, use long long c1, c2
        /// u -> v: c1
...
struct SuffixArray {
        ...
        , M(L)
    {
        for (int i = 0; i < L; i++)
            P[0][i] = int(s[i]);
        if (L == 1)
            P[0][0] = 0; /// very important :)))), otherwise it may cause an error when L = 1
...
struct MaxBipartiteMatching { /// base 1th; X -> Y
    ...
    int getX(int y)
    { /// match[y] = x, if have x -> y in MaxBipartiteMatching or 0 if not
        return match[y];
    }
};
...
struct Trie_Binary {
    ...
    bool find(int x)
    { /// if we have a number = x then return true, else return false
        int root = 0;
        for (int i = Base - 1; i >= 0; i--) {
            int bit = getBit(x, i);
            if (T[root][bit] == 0)
...
struct Trie_String {
    ...
    bool find(string s)
    { /// if we have a complete string t = s then return true, else return false
        int root = 0;
        for (int i = 0; i < (int)s.size(); i++) {
            int c = s[i] - 'a'; /// change if s not contain only alphabet
            assert(c >= 0 && c < Base);
    ...
    { /// if we have a complete string t so s is the prefix in t then return true, else return false
        int root = 0;
        for (int i = 0; i < (int)s.size(); i++) {
            int c = s[i] - '0';
            if (T[root][c] == 0)
...
struct DisjointSetUnion { /// Base 1th or Base 0th
    ...
    int find_root(int u)
    { /// find the root in the Set contain u
        if (p[u] < 0)
            return u; /// u is root
    ...
    void Union(int u, int v)
    {
        ...
        if (x == y)
            return;
        if (p[x] > p[y])
            swap(x, y); /// small Set point to big Set, p[x] > p[y] so |p[x]| < |p[y]|
        p[x] += p[y];
        p[y] = x;
    }
...
}
{
    ...
    long long GetPalindrome(int K)
    { /// tìm số Palindrome thứ K (1, 2, 3, ..., 9, 11, 22, 33, ...)
        if (K < 10)
            return K;
        int n = 1;
        while (true) {
...
}
{
    ...
    long long F(int p, int q, int n)
    { /// Tinh tổng floor(p * i / q) với i từ 1 đến n và p < q
        if (p == 0)
            return 0;
...
{ /// Tinh to hop chap K cua N voi K, N <= 2 x 10 ^ 6
    for (int i = 1; i <= N; i++) {
        F[i] = 0;
    }
    if (K > N) {
    ...
    string compactExpression(string s)
    {
        ...
        for (auto [x, op] : exp) {
            if (op == '?')
                ...
                if (op == '-')
                    result = sub(prev, last);
                if (op == '*')
                    result = mul(prev, last);
                if (op == '^')
                    result = power(prev, last);
